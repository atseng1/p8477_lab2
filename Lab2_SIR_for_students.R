...
parameters = c(beta = 0.5, gamma = 0.3);  # store the model parameters
...
sim_3 = ode(y = state, times = times, func = SIR_2, parms = parameters);
...
wklyS[1] = S0; wklyI[1] = I0; wklyR[1] = R0;
wk = 1; # first week
times = seq(tmstep*(wk-1),tmstep*wk,by=1);
...
times = seq(0, 365, by = 1);  # run for a year
state = c(S = S0, I = I0, R = R0)
sim_4 = ode(y = state, times = times, func = SIR_2, parms = parameters);
...
times = seq(0, 365, by = 1);  # run for a year
state = c(S = S0, I = I0, R = R0)
sim_5 = ode(y = state, times = times, func = SIR_2, parms = parameters);
...
times = seq(0, 365, by = 1);  # run for a year
state = c(S = S0, I = I0, R = R0)
sim_6 = ode(y = state, times = times, func = SIR_2, parms = parameters);
...
# As gamma increases, the recovered rate decreases.
...
WkExp = 3; # CHANGE THE NUMBER OF WEEK HERE
